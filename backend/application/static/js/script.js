document.addEventListener("DOMContentLoaded", function () {
   ...
   ==================================================== */
	...
	async function loadConversations() {
		try {
			...
			if (GetConversaID() != "new")
				//Atualiza currentConversation com o valor da conversa atual
				currentConversation = conversations.find(
					(conv) => String(conv.pk) === String(GetConversaID())
				);
		} catch (error) {
	...
	if (GetConversaID() != "new") {
		//Se a conversa não for nova, obtém as mensagens
	...
	function renderConversationsSidebar() {
		if (GetConversaID() != "new") {
			//Se a conversa não for nova, exibe item de nova conversa sem background color de destaque
      ...
      `;
		} //Se a conversa for nova, exibe item de nova conversa com background color de destaque
		else {
			historyList.innerHTML = `
      ...
      `;
		...
		conversations.forEach((conv) => {
			...
			li.setAttribute(
				"onclick",
				`window.location.href='/chat/${conv.pk}'`
			); //Cria link que redireciona o usuário para a conversa ao clicá-la na sidebar

			if (GetConversaID() == conv.pk) {
				//Coloca a conversa atual com um background color de destaque
								...
								try {
									...
									if (data.success) {
										...
										console.error(
											...
											data.error
										); //Exibe Error Message acima das mensagens
										ShowErrorMessage(
											"Houve algum erro na conexão. Não foi possível trocar o nome da conversa"
										);
									}
								} catch (error) {
		...
		async function changePrompt() {
			try {
				...
				if (data.success) {
					...
					ShowErrorMessage(
						"Houve algum erro na conexão. Não foi possível alterar o prompt de instrução"
					);
				}
			} catch (error) {
	...
	function showDeleteFileModal(doc_id, doc_name) {
		...
		const DeleteFileModalBody = document.getElementById(
			"deleteFileModalBody"
		);
		const DeleteFileCancelBtn = document.getElementById(
			"deleteFileCancelBtn"
		);
	...
	function createUserMessage(msgDiv, msg) {
		...
		if (msg.fields.documento !== null) {
			...
			else link = `/media/upload/${slugify(msg.fields.documento.titulo)}`;
	...
	function renderMessages() {
		...
		mensagens.forEach((msg) => {
			...
			if (msg.pk !== null) msgDiv.setAttribute("message-id", msg.pk);

			if (msg.fields.eh_do_usuario) {
				//Se a mensagem é do usuário, marca como do usuário (CSS aplica os estilos diferentes)
	...
	function startLoadingAnimation() {
		...
		animationInterval = setInterval(() => {
			dots.forEach((dot, i) => {
				if (i === index) {
					dot.style.animation =
						"bounce .6s cubic-bezier(0.6, 0.1, 1, 1.4) infinite";
	...
	sendBtn.addEventListener("click", () => {
		...
		if (isSending == true)
			return; // Se outra mensagem estiver sendo enviada, não faz nada
		else if (messageText == "")
			return; // Se a mensagem for vazia, não faz nada
		else if (messageText.length > MAX_CHARACTERS) {
			//Se a mensagem exceder o limite de caracteres, exibe mensagem de aviso no topo da tela
			...
			messageInput.value = "";
		} // Se a mensagem conter um número de caracteres dentro dos limites, esvazia o Input
		...
		if (has_file) {
			...
			mensagens.push({
				fields: {
					texto: messageText,
					eh_do_usuario: true,
					documento: { titulo: file.name },
				},
			}); // Guarda mensagem do usuário com PDF
		} else {
			mensagens.push({
				fields: {
					texto: messageText,
			...
			}); // Guarda mensagem do usuário se não houver PDF
		}
		...
		async function enviarMensagem(conv_id) {
			try {
				...
				if (data.success) {
					...
					if (GetConversaID() === "new") {
						// Se a conversa for nova, redireciona para outra URL
						window.history.pushState({}, "", data.redirect);
						conversations.unshift({
							fields: {
								nome: data.nome_da_nova_conversa,
							},
							pk: data.nova_conversa_id,
						}); //Adiciona conversa na lista de conversas
						renderConversationsSidebar();
					}
					mensagens.push({
						fields: {
					...
					if (has_file) {
						// Se há um arquivo PDF anexado, recarrega todas as mensagens da conversa para exibir o caminho do PDF correto
						mensagens[
							mensagens.length - 2
						].fields.documento.arquivo_url = `/show-pdf/${data.documento.pk}`;
						documents.push({
							fields: {
								titulo: data.documento.titulo,
							},
							pk: data.documento.pk,
						});
						renderMessages();
					} else {
					...
					ShowErrorMessage(
						"Houve algum erro na conexão. Não foi possível enviar a mensagem"
					);
				}
			} catch (error) {
	...
	fileInput.addEventListener("change", () => {
        ...
        `;
				...
				ViewFile.innerHTML =
					"<h1>Por favor, selecione um arquivo PDF válido.</h1>";
